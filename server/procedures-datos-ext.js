"use strict";
const VarCal = require("varcal");
const fs = require("fs-extra");
const likear = require("like-ar");
//GENERALIZAR
const OPERATIVO = 'REPSIC';
const ESTRUCTURA_PARA_GENERAR = {
    aliases: {
        padre: {
            tabla: 'personas',
        ...
        coreFunction: async function (context, parameters) {
                    ...
                    WHERE EXISTS 
                        (SELECT variable FROM variables v 
                            WHERE v.operativo=op.operativo and v.variable=op.variable 
                                and v.clase='relevamiento' and v.operativo=$1)`, [parameters.operativo]).execute();
            await context.client.query(`DELETE FROM repsic.variables WHERE operativo = $1 and clase = 'relevamiento'`, [parameters.operativo]).execute();
            await context.client.query(`INSERT INTO repsic.variables(
              ...
              select c1.operativo, var_name, c0.unidad_analisis, 
                ...
                order by orden_total`, [parameters.operativo]).execute();
            await context.client.query(`
                ...
                INSERT INTO repsic.variables_opciones(
                  ...
                  select op.operativo, pre.var_name, casillero::integer,op.nombre, orden
                    ...
                    order by orden_total, orden`, [parameters.operativo]).execute();
